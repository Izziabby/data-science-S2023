...
* Within the script editor or a chunk
    ...
    * `Cmd/Ctrl` + `Enter` = run line/selection


    * `F1` (Note: on a Mac you need to press `fn` + `F1`) = Show help for function at cursor


	* `Cmd/Ctrl` + `Shift` + `C` = comment/uncomment line/selection

* `Cmd/Ctrl` + `Shift` + `K` = Show markdown

* `Cmd/Ctrl` + `Shift` + `R` = Run all
...
* Within a chunk
    ...
    * `Ctrl` + `I` (`Cmd` + `I`) = re-indent selected lines
...
red square top right corner of chunk
or esc
