...
df_psaap %>% 
  ggplot() +
  geom_line(mapping = aes(x, T_norm, color = as.factor(idx)), show.legend = FALSE)
```
...
df_acc <- data.frame(df_train_mse = c(mse(fit_baseline, df_train), 
                                      mse(fit_cheat, df_train), 
                                      mse(fit_nonphysical, df_train)
                                      ),
                     df_validate_mse = c(mse(fit_baseline, df_validate), 
                                      mse(fit_cheat, df_validate), 
                                      mse(fit_nonphysical, df_validate)
                                      ),
                     df_train_rsquare = c(rsquare(fit_baseline, df_train), 
                                      rsquare(fit_cheat, df_train), 
                                      rsquare(fit_nonphysical, df_train)
                                      ),
                     df_validate_rsquare = c(rsquare(fit_baseline, df_validate), 
                                      rsquare(fit_cheat, df_validate), 
                                      rsquare(fit_nonphysical, df_validate)
                                      )
                     )
  
rownames(df_acc) <- c("fit_baseline","fit_cheat","fit_nonphysical")
df_acc
```

    ##                 df_train_mse df_validate_mse df_train_rsquare
    ## fit_baseline      0.15153384      0.08092764      0.348376352
    ## fit_cheat         0.05194415      0.05371774      0.776630527
    ## fit_nonphysical   0.23224544      0.15905165      0.001301518
    ##                 df_validate_rsquare
    ## fit_baseline            0.474654560
    ## fit_cheat               0.637405082
    ## fit_nonphysical         0.001898415

**Observations**:

- Which model is *most accurate*? Which is *least accurate*?
  - the cheat model is the *most* accurate (lower mse and higher rsquare
    is more accurate)
  - the nonphysical model is the *least* accurate
- What *Category* of variable is `avg_T`? Why is it such an effective
  predictor?
  - it is in the Output *Category*
  - it is an effective predictor because avg_T is used to calculate
    T_norm
- Would we have access to `avg_T` if we were trying to predict a *new*
  value of `T_norm`? Is `avg_T` a valid predictor?
  - both variables would not be available in a prediction
  - No, avg_T is not a valid predictor
- What *Category* of variable is `idx`? Does it have any physical
  meaning?
  - it is Metadata
  - It has no physical meaning, it indicates the simulation number
...
fit_q4 <- 
  df_train %>% 
  lm(formula = T_norm ~ x + d_p + L + U_0 + N_p + C_pv)
...
- x high rsquared value from q3

- d_p and U_0 individually largest rsquared values

- N_p C_pv gave much higher value after trying different options
...
- Which model tends to be more accurate? How can you tell from this
  ...
  - my model from q4 seems to be more accurate because the points follow
    the trend line where the predicted value is close to the actual
    value
- Which model tends to be *more confident* in its predictions? Put
  differently, which model has *narrower prediction intervals*?
  - my q4 model seems to be more confident because it has narrower
    prediction intervals
- How many predictors does the `fit_simple` model need in order to make
  a prediction? What about your model `fit_q4`?
  - fit_simple used one predictor
  - fit_q4 used 6 predictors
...
val_cases <- df_validate %>% 
  ...
  summarise(n_val = sum(in_range), n_sum = n()) %>% 
  mutate(frac = n_val / n_sum)
...
- How much do you trust your model? Why?
  - I trust this model as 93% of the validation data falls within the
    predicted range and the rsquared value is approximately 0.67
- What kind of interval—confidence or prediction—would you use for this
  task, and why?
  - I would use prediction interval because it summarizes information
    about future observations which is the goal of our model
  - Additionally, PI is useful when we care more about individual
    observations—rather than the aggregate behavior
- What fraction of validation cases lie within the interval you predict?
  How does this compare with `pr_level`?
  - 56/60 cases lie within the interval we predicted
  - the pr_level was supposed to be .8 which is lower than our
    prediction
- What interval for `T_norm` would you recommend the design team to plan
  around?
  - The team should plan around 1.45685-2.296426
- Are there any other recommendations you would provide?
  - look at other variable to see which makes sense and which can make
