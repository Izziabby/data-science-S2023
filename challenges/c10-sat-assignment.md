...
| Column     | Meaning                               |
|------------|---------------------------------------|
| `high_GPA` | high school grade point average (gpa) |
| `math_SAT` | math SAT score                        |
| `verb_SAT` | Verbal SAT score                      |
| `comp_GPA` | Computer science gpa (in university)  |
...
- What kinds of information *do we not have* about these students?
  ...
  - high school verbal/ english class gpa
  - college math gpa
  - college verbal/ english class gpa
  - practice test score(s)
  - other SAT scores
  - how long they studied for/ study habits
  - financial situation
- Based on these missing variables, what possible effects could be
  present in the data that we would have *no way of detecting*?
  - student improvement on the SAT over time
...
df_composite %>% 
  ggplot(aes(x = univ_GPA)) +
    ...
    geom_point(aes(y = both_SAT/coeff), color = "red") +
  scale_y_continuous(
...
- What relationship do `univ_GPA` and `both_SAT` exhibit?
  - as gpa increase so does the composite score
- What relationship do `univ_GPA` and `high_GPA` exhibit?
  - the higher the gpa in university, the higher the gpa was in high
    school
...
corr_high <- cor.test(df_composite$high_GPA, df_composite$univ_GPA)

corr_both <- cor.test(df_composite$both_SAT, df_composite$univ_GPA)
...
- Which correlations are significantly nonzero?
  - both correlations are significantly non-zero
- Which of `high_GPA` and `both_SAT` seems to be more strongly
  correlated with `univ_GPA`?
  - high_GPA seems to be more strongly correlated
- How do the results here compare with the visual you created in q2?
  - This makes sense because high_GPA has a large bump in the same area
    as univ_GPA
- Based on these results, what can we say about the predictive
  capabilities of both `high_GPA` and `both_SAT` to predict `univ_GPA`?
  - high_GPA seems to be better at predicting univ_GPA since they are
...
high_GPA_resample <-
  bootstraps(df_composite$high_GPA, times = 1000)
